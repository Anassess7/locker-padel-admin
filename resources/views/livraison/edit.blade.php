...
<div class="max-w-4xl mx-auto">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">Modifier la Livraison</h1>
    ...
    <form action="{{ route('livraisons.update', $livraison) }}" method="POST" class="bg-white rounded-lg shadow p-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <select name="client_id" id="client_id" class="w-full rounded-md border-gray-300" required>
                    ...
                    @foreach($clients as $client)
                        <option value="{{ $client->id }}" {{ old('client_id', $livraison->client_id) == $client->id ? 'selected' : '' }}>
                            {{ $client->nom_client }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="date" name="date_livraison" id="date_livraison" 
                    value="{{ old('date_livraison', $livraison->date_livraison->format('Y-m-d')) }}" 
            ...
            <div>
                ...
                <input type="text" name="company" id="company" value="{{ old('company', $livraison->company) }}" 
                    class="w-full rounded-md border-gray-300" required>
                @error('company')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="text" name="reference" id="reference" value="{{ old('reference', $livraison->reference) }}" 
                    class="w-full rounded-md border-gray-300" required>
                @error('reference')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" step="0.01" name="prix_livraison" id="prix_livraison" 
                    value="{{ old('prix_livraison', $livraison->prix_livraison) }}" 
                    class="w-full rounded-md border-gray-300" required>
        ...
        <div class="mb-6">
            ...
            <div id="articles-container">
                @foreach($livraison->articles as $index => $article)
                <div class="article-item grid grid-cols-1 md:grid-cols-4 gap-4 mb-4 p-4 border rounded">
                    ...
                    <div>
                        ...
                        <input type="number" name="articles[{{ $index }}][quantite]" 
                            class="w-full rounded-md border-gray-300 quantite-input" 
                            min="1" value="{{ $article->pivot->quantite }}" required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Prix Unitaire</label>
                        <input type="number" step="0.01" name="articles[{{ $index }}][prix_unitaire]" 
                            class="w-full rounded-md border-gray-300 prix-input" 
                            value="{{ $article->pivot->prix_unitaire }}" required>
                    </div>
        ...
        <div class="mb-6 p-4 bg-gray-50 rounded">
            <div class="flex justify-between items-center">
                ...
                <span id="total-price" class="text-xl font-bold">{{ number_format($livraison->prix_total, 2, ',', ' ') }} Dhs</span>
            </div>
        </div>
    ...
    <div class="article-item grid grid-cols-1 md:grid-cols-4 gap-4 mb-4 p-4 border rounded">
        <div>
            ...
            <select name="articles[INDEX][id]" class="w-full rounded-md border-gray-300 article-select" required>
                <option value="">Sélectionner un article</option>
                @foreach($articles as $art)
                    <option value="{{ $art->id }}" data-prix="{{ $art->prix_vente }}">
                        {{ $art->nom }}
                    </option>
                @endforeach
            </select>
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Edit form DOM loaded, initializing...');
    ...
    function calculateTotal() {
        ...
        // Update display - remove formatting and add simple format
