...
<div class="flex min-h-screen bg-gray-100">
    ...
    <main class="flex-1 p-8">
        <h1 class="text-2xl font-bold mb-6">Ajouter Utilisateur</h1>
        <form action="{{ route('utilisateurs.store') }}" method="POST" class="bg-white p-6 rounded shadow max-w-lg">
            @csrf
            <div class="mb-4">
                <label class="block mb-2 font-semibold">Nom d'utilisateur</label>
                <input type="text" name="nom_utilisateur" class="w-full border rounded px-3 py-2" required>
            </div>
            <div class="mb-4">
                <label class="block mb-2 font-semibold">Mot de passe</label>
                <input type="password" name="mdp" class="w-full border rounded px-3 py-2" required>
            </div>
            <div class="mb-4">
